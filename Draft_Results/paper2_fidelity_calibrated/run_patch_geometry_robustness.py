...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Patch-geometry robustness for a fixed selected feature set.")
...
def main() -> int:
    ...
    out_root = (
        ...
        else PROJECT_ROOT / "paper2_fidelity_calibrated" / "results" / "patch_geometry_robustness" / str(args.pair) / str(args.model)
    )
    ...
    geometries = [
        ("raw", None, None),
        ("clipped", 1.0, None),
        ("sign_normalized", None, 1.0),
    ]
    item_rows: List[Dict[str, Any]] = []

    log(f"Running patch-geometry robustness: pair={args.pair} model={args.model} items={len(eval_rows)}")
    ...
    for item_idx, word in enumerate(eval_rows, start=1):
        ...
        for geometry, clip_cap, sign_scale in geometries:
            result = run_patching_experiment(
                ...
                patch_geometry_clip_cap=clip_cap,
                patch_geometry_sign_scale=sign_scale,
    ...
    for geometry, _, _ in geometries:
        rows = [row for row in item_rows if str(row.get("geometry")) == str(geometry)]
        summary[str(geometry)] = {
    ...
    payload = {
        "experiment": "patch_geometry_robustness",
    ...
    _write_json(out_root / "patch_geometry_robustness.json", payload)
    log(f"Saved: {out_root / 'patch_geometry_robustness.json'}")
    return 0
