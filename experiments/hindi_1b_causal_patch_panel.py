...
def main() -> int:
    ...
    print(
        f"Running Hindi causal patch panel: model={args.model} pair={args.pair} items={len(eval_rows)} layers={layers} components={components}",
        flush=True,
    )
    ...
    for item_idx, word in enumerate(eval_rows, start=1):
        ...
        for recipient_condition, donor_condition in DEFAULT_PATCHES:
    ...
    payload = {
        ...
        "patches": [{"recipient_condition": r, "donor_condition": d} for r, d in DEFAULT_PATCHES],
